    ...
    <>
      ...
      <div id="support-platform-pill" className={`fixed z-40 transition-all ${
        ...
        >
          ...
          <span className="text-xs font-bold tracking-tight text-[#0c2217]">Support Platform</span>
          <span className="hidden sm:inline text-[10px] font-mono text-[#b85d34] bg-[#f5f0e6] px-2 py-0.5 rounded-full border border-[#d4a359]/40 font-bold">
            Sadaqah Jariyah
          </span>
      ...
      {/* 2. Full Modal Dialog with All 4 Payment Barcodes (Light & Illuminated) */}
      {isOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-2.5 sm:p-4 bg-black/35 backdrop-blur-sm animate-in fade-in duration-200">
          ...
          >
            ...
            {/* Modal Header */}
            <div className="p-4 sm:p-5 border-b border-[#ebe3d3] flex items-center justify-between shrink-0 bg-[#faf8f5] z-10">
              <div className="flex items-center gap-3">
                <div className="p-2 sm:p-2.5 rounded-2xl bg-rose-50 text-rose-600 border border-rose-200 shadow-xs">
                  <Heart className="w-5 h-5 fill-rose-500/20" />
                </div>
                <div>
                  <h3 className="font-serif font-black text-base sm:text-lg text-[#0c2217] leading-tight">
              ...
              <button
                ...
                className="p-2 rounded-full text-stone-400 hover:text-[#0c2217] hover:bg-[#ebe3d3] transition-colors cursor-pointer"
              >
                <X className="w-5 h-5" />
              </button>
            </div>
            ...
            <div className="p-3.5 sm:p-5 space-y-4 overflow-y-auto flex-1 overscroll-contain">
              ...
              <div>
                <p className="text-[11px] font-bold text-[#0c2217] uppercase tracking-wider mb-2">
                      ...
                      <button
                        ...
                        className={`p-2.5 rounded-2xl border text-center transition-all cursor-pointer flex flex-col items-center justify-center gap-0.5 ${
                          isSelected
                            ...
                            : 'bg-[#f5f0e6] border-[#ebe3d3] text-[#4a5e55] hover:bg-white hover:border-[#d4a359]'
                        }`}
                      >
                        <span className={`text-[11px] sm:text-xs font-black block leading-tight ${isSelected ? 'text-white' : 'text-[#0c2217]'}`}>
                          {method.shortName}
                        </span>
                        <span className={`text-[9px] font-medium block ${isSelected ? 'text-amber-200' : 'text-[#6b7f76]'}`}>
                          {method.category}
                        </span>
                      </button>
                    );
              ...
              <div className="rounded-2xl bg-[#faf8f5] border border-[#ebe3d3] p-3.5 sm:p-4 flex flex-col sm:flex-row items-center gap-4 shadow-xs">
                
                {/* QR Code Frame */}
                <div className="p-2.5 bg-white rounded-2xl shadow-md border-2 border-[#d4a359]/40 shrink-0 relative group">
                  <img
                ...
                <div className="flex-1 text-center sm:text-left space-y-2.5 w-full">
                  ...
                  {/* Method Header Badge */}
                  <div className="inline-flex items-center gap-1 px-3 py-1 rounded-full text-xs font-bold bg-emerald-50 border border-emerald-200 text-emerald-800">
                    <CheckCircle2 className="w-3.5 h-3.5 text-emerald-600" />
                    <span>{selectedMethod.name}</span>
                  </div>

                  {/* Account Title */}
                  <div className="p-2.5 rounded-xl bg-white border border-[#ebe3d3] text-left">
                    <p className="text-[9px] uppercase tracking-wider font-bold text-stone-500">
                      Account Title
                    </p>
                    <p className="text-xs sm:text-sm font-black text-[#0c2217] mt-0.5 tracking-wide font-serif">
                      {selectedMethod.accountTitle}
                    </p>
                  </div>

                  {/* Account Number with 1-Click Copy */}
                  <div className="p-2.5 rounded-xl bg-white border border-[#d4a359]/60 text-left flex items-center justify-between gap-2 shadow-2xs">
              ...
              <div className="p-3 sm:p-3.5 rounded-2xl bg-[#f5f0e6] border border-[#ebe3d3] space-y-2">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-1.5">
                    <CreditCard className="w-3.5 h-3.5 text-[#b85d34]" />
                    <span className="text-[11px] font-bold uppercase tracking-wider text-[#0c2217]">
                      Direct Transfer Ledger
                    </span>
                  </div>
                  <span className="text-[10px] text-stone-500 font-semibold">Title: Abdul Khaliq</span>
                </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 text-xs">
                  ...
                  <div className="p-2.5 rounded-xl bg-white border border-[#ebe3d3] flex items-center justify-between gap-2 shadow-2xs">
                    <div>
                      <span className="text-stone-500 text-[9px] font-bold block uppercase tracking-wide">Meezan Bank</span>
                      <span className="font-mono font-black text-[#0c2217] text-xs">96010105435308</span>
                    </div>
                    <button
                      type="button"
                      onClick={() => handleCopy('96010105435308', 'summary-meezan')}
                      className="p-1.5 hover:bg-[#f5f0e6] rounded-lg text-[#b85d34] transition-colors cursor-pointer shrink-0"
                  ...
                  <div className="p-2.5 rounded-xl bg-white border border-[#ebe3d3] flex items-center justify-between gap-2 shadow-2xs">
                    <div>
                      <span className="text-stone-500 text-[9px] font-bold block uppercase tracking-wide">EasyPaisa / JazzCash / UPaisa</span>
                      <span className="font-mono font-black text-[#0c2217] text-xs">03171759093</span>
                    </div>
                    <button
                      type="button"
              ...
              <div className="p-3 rounded-2xl bg-amber-50/80 border border-[#d4a359]/40 text-[11px] text-[#4a3b10] flex items-start gap-2.5 leading-relaxed">
                <ShieldCheck className="w-4 h-4 text-[#b85d34] shrink-0 mt-0.5" />
                <span>
            ...
            {/* Modal Footer */}
            <div className="p-3.5 sm:p-4 border-t border-[#ebe3d3] flex items-center justify-between shrink-0 bg-[#faf8f5] z-10">
              <span className="text-[11px] text-stone-500 font-medium hidden sm:inline">
                Abdul Khaliq &bull; Meezan Bank &amp; Mobile Wallets
              </span>
              <button
                onClick={() => setIsOpen(false)}
                className="w-full sm:w-auto px-6 py-2 rounded-xl bg-[#b85d34] hover:bg-[#9e4e2a] border border-[#d4a359]/40 text-xs font-bold text-white transition-all cursor-pointer text-center shadow-md hover:scale-[1.02] active:scale-95"
              >
                Close Dialog
              </button>
            </div>
