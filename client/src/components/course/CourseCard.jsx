...
export default function CourseCard({ course }) {
  ...
  return (
    <div className="bg-white rounded-3xl border border-[#e6ded1] shadow-2xs hover:shadow-xl hover:border-[#b85d34]/40 transition-all duration-200 flex flex-col justify-between overflow-hidden group">
      
      {/* Top Banner Image with Badges */}
      <div className="relative h-44 w-full bg-[#0c2217] overflow-hidden">
        <img
        ...
        <div className="absolute top-3 left-3 right-3 flex items-center justify-between gap-2">
          ...
          <span className="px-2.5 py-1 rounded-full text-[10px] font-bold bg-black/60 text-[#d4a359] border border-[#d4a359]/30 backdrop-blur-xs flex items-center gap-1">
        ...
        <div className="absolute bottom-3 left-3 right-3">
          <span className="text-[10px] font-semibold text-[#d4a359] flex items-center gap-1">
      ...
      <div className="p-5 flex-1 flex flex-col justify-between space-y-4">
        ...
        <div className="space-y-2">
          <Link href={`/courses/${course.slug}`}>
            <h3 className="text-base font-serif font-black text-slate-900 group-hover:text-[#0c2217] transition-colors line-clamp-2 leading-snug">
              {course.title}
            </h3>
          </Link>
        ...
        {/* Instructor Row */}
        {course.instructor && (
          <div className="flex items-center gap-2.5 pt-2 border-t border-[#e6ded1]">
            <img
        ...
        {/* Metrics Grid */}
        <div className="grid grid-cols-3 gap-1.5 p-2 bg-[#faf8f5] rounded-2xl text-center border border-[#e6ded1]">
          <div>
          ...
          <div className="border-x border-[#e6ded1]">
            <span className="text-[10px] font-bold text-slate-400 uppercase block">Lessons</span>
            <span className="text-xs font-black text-[#0c2217]">{totalLessons}</span>
          </div>
          <div>
            <span className="text-[10px] font-bold text-slate-400 uppercase block">Quizzes</span>
            <span className="text-xs font-black text-[#b85d34]">{totalTests}</span>
          </div>
        </div>

        {/* Price & CTA Button */}
        <div className="flex items-center justify-between pt-2 border-t border-[#e6ded1]">
          <div>
