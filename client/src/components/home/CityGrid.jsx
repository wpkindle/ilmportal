...
const CityGrid = () => {
  ...
  return (
    <section className="py-24 relative overflow-hidden bg-white border-b border-slate-200/80 bg-dot-matrix">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
          {majorCities.map((city) => (
            <Link
              ...
              className="p-5 rounded-3xl bg-slate-50 border border-slate-200/80 hover:bg-emerald-50/50 hover:border-emerald-300 hover:shadow-md transition-all group space-y-2"
            >
              <div className="flex items-center justify-between">
                <div className="p-2 bg-emerald-100 text-emerald-700 rounded-xl group-hover:scale-110 transition-transform">
                  <MapPin className="w-4 h-4" />
                </div>
                <span className="text-[10px] font-bold uppercase px-2 py-0.5 bg-slate-200 text-slate-700 rounded-md">
                  {city.province}
                </span>
              </div>
              <div>
