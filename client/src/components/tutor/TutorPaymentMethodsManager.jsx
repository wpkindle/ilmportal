...
export default function TutorPaymentMethodsManager({
  ...
  const effectivePreviewUrl = previewUrl || (tutorId ? `/tutors/${tutorId}` : null);
        ...
        <div className="p-4 bg-amber-50/80 border border-amber-200/80 rounded-2xl flex items-start gap-3 text-xs text-amber-900">
          ...
          <div className="space-y-1">
            <div className="flex items-center gap-2 flex-wrap">
              <h4 className="font-bold text-amber-950">Add Personal Accounts (Optional) or Use Administration Accounts</h4>
              <span className="px-2 py-0.5 text-[9.5px] font-bold rounded-full bg-emerald-100 text-emerald-800 border border-emerald-300">
                Not Required for 100% Health
              </span>
            </div>
            <p className="text-amber-800 leading-relaxed text-[11px]">
              Adding personal accounts is completely optional and does not reduce your profile completion score. You can add Pakistani receiving accounts (Bank, Raast ID, EasyPaisa, JazzCash, UPaisa) at any time. Alternatively, you can always choose <strong>Administration Accounts</strong> when dispatching tuition fee requests so students pay through verified platform accounts.
            </p>
            <button
              type="button"
              onClick={openAddModal}
            ...
            >
              <span>+ Add a personal payment method (Optional)</span>
