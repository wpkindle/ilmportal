...
export const EDITORIAL_ARTICLES = [
  {
    ...
    author: 'Abdul Khaliq',
    authorRole: 'Co-Founder & Platform Director',
...
*Co-Founder & Platform Director, IlmiDunya Pakistan*`
  },
  {
    ...
    authorRole: 'Co-Founder & Female Safety Dean',
...
*Co-Founder & Female Safety Dean, IlmiDunya Pakistan*`
  },
  {
    ...
    author: 'Guest Author',
    authorRole: 'Contributing Scholar & Academic Specialist',
...
// Helper to fetch all editorial articles
export const getEditorialArticles = () => {
  return EDITORIAL_ARTICLES;
};

// Helper to fetch a single editorial article by slug
export const getEditorialArticleBySlug = (slug) => {
  if (!slug) return null;
  const cleanSlug = String(slug).trim().toLowerCase();
  
  // Exact match first
  let article = EDITORIAL_ARTICLES.find(
    (a) => a.slug.toLowerCase() === cleanSlug
  );
  if (article) return article;

  // Fuzzy match (contains or starts with)
  article = EDITORIAL_ARTICLES.find(
    (a) => a.slug.toLowerCase().includes(cleanSlug) || cleanSlug.includes(a.slug.toLowerCase())
  );
  if (article) return article;

  // Fallback: match by title keywords or default to the first article if matching camera-off / sanad
  if (cleanSlug.includes('camera') || cleanSlug.includes('sanad') || cleanSlug.includes('foundations')) {
    return EDITORIAL_ARTICLES[0];
  }
  if (cleanSlug.includes('alimah') || cleanSlug.includes('daughter') || cleanSlug.includes('modesty')) {
    return EDITORIAL_ARTICLES[1];
  }
  if (cleanSlug.includes('tajweed') || cleanSlug.includes('mistake')) {
    return EDITORIAL_ARTICLES[2];
  }
